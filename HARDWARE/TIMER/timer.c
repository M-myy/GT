...
void TIM2_IRQHandler(void)  //定时器2中断服务函数
{
	if(TIM_GetITStatus(TIM2, TIM_IT_Update))  //如果为溢出中断
	{
		...
		TIM_ClearITPendingBit(TIM2, TIM_IT_Update);  //请吃中断标志
	}
}
...
void TIM5_IRQHandler(void)     //理论上电机测速时该中断函数根本用不到，因为0.5秒的定时器2等不到定时器5溢出就会清除TIM5->CNT，为保险起见加上
{
...
void TIM4_init(unsigned int arr,unsigned int psc)	//定时器初始化函数
{
	...
	NVIC_InitStru.NVIC_IRQChannel=TIM4_IRQn;  //选择定时器4中断
	NVIC_InitStru.NVIC_IRQChannelCmd=ENABLE;  //使能中断
	NVIC_InitStru.NVIC_IRQChannelPreemptionPriority=1;  //中断优先级为1
	NVIC_InitStru.NVIC_IRQChannelSubPriority=2;  //中断子优先级为2
	NVIC_Init(&NVIC_InitStru);   //初始化中断配置
	
	TIM_Cmd(TIM4,DISABLE);  //先关闭显示
}

void TIM4_IRQHandler(void)
{
  static int i = 0;
	static float LAST_SPEED = 0.0;  //储存上一次速度值
	
	if(TIM_GetITStatus(TIM4, TIM_IT_Update))  //判断是否为更新时间中断
	{
	    TIM_ClearITPendingBit(TIM4, TIM_IT_Update);  //清除中断标志位
	    LCD_Fill(10+i,200,10+i,300,WHITE);  //打点
	    i++;
      if(i>=220)i = 0;
