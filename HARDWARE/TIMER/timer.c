...
void TIM5_Cap_Init(unsigned int arr,unsigned int psc)  //定时器5初始化函数
{
	...
	TIM_TimeBaseInit(TIM5,&TIM_TimeBaseInitStru);
  ...
  TIM_ICStructInit(&TIM_ICInitStru);
	...
	NVIC_InitStru.NVIC_IRQChannel=TIM5_IRQn;
...
void TIM4_init(unsigned int arr,unsigned int psc)	//定时器初始化函数
{
	...
	TIM_TimeBaseInitStru.TIM_Prescaler=psc;
	TIM_TimeBaseInitStru.TIM_Period=arr;
	TIM_TimeBaseInit(TIM4,&TIM_TimeBaseInitStru);
	
	TIM_ITConfig(TIM4,TIM_IT_Update,ENABLE);
	
	NVIC_InitStru.NVIC_IRQChannel=TIM4_IRQn;
	NVIC_InitStru.NVIC_IRQChannelCmd=ENABLE;
